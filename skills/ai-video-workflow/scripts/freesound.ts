...
import { formatFreesoundComments, formatFreesoundResults } from "./formatters";
...
async function searchSounds(flags: Record<string, string>) {
  ...
  if (flags["--output"]) {
    ...
    console.log(formatFreesoundResults(data));
  }
}
...
async function getSound(flags: Record<string, string>) {
  ...
  if (flags["--output"]) {
    ...
    console.log(formatFreesoundResults({ count: 1, results: [data] }));
  }
}
...
async function getComments(flags: Record<string, string>) {
  ...
  if (flags["--output"]) {
    ...
    console.log(formatFreesoundComments(data));
  }
}
